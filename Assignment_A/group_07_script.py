...
prev_reading = [100,500,500,0,0,20,8,40,0,200]

is_periodical_values = [1,1,1,0,0,1,1,1,0,1]
threshold_values = [0.3,0.3,0.3,0.75,0.75,0.3,0.3,0.3,0.1,0.3]
min_values = [10,20,20,0,0,15,-15,0,0,0]
max_values = [5000,6000,6000,1,1,45,30,100,0,400]
...
def publish_function(name,seed, cycles,sensor_val):
    ...
    for cyc in range(cycles):
        ...
        # print(str(current_reading)+'\n')
        if is_periodical_values[sensor_val]:
            client.publish(current_topic,current_reading)
            time.sleep(sensor_intervals[sensor_val])
        else:
            if current_reading != prev_reading[sensor_val] :
                client.publish(current_topic,current_reading)
            prev_reading[sensor_val] = current_reading
...
if __name__ == "__main__":
    ...
    for t in range(1,11):
        ...
        threads.append(threading.Thread(target=publish_function,args=(t,t,10,t-1)))
